...
class MenuBarController(QObject):
    ...
    def _create_settings_menu(
        ...
        if not getattr(constants, "SINGLE_PLUGIN_MODE", False):
            ...
            # Deliberately no PreferencesRole: macOS merges such an item into the
            # application menu's standard entry, which forces the title to
            # "Settings…" with no way to keep the real label. That slot belongs to
            # an app-wide settings dialog, which this framework does not have yet.
