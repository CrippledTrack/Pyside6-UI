...
settings merge, skip-unchanged writes, future-schema handling, HostConfig paths,
tab labels vs plugin_id, overlapping-ownership lifecycle (loader signal, disable
shutdown order, daemon reader, theme custom-vs-builtin, notification unsubscribe,
thread-free daemon ping, theme apply callback, tab load error view),
PyInstaller hidden imports for dynamic UI maps, Linux Qt xcb deps (apt / dnf /
pacman detected by tool presence, not distro ID), and the current synchronous
dialog contracts. Theme JSON export stores stylesheets as line arrays and still loads
