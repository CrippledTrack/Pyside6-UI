...
def _get_git_commit(cwd: Path) -> str:
    """Run git rev-parse HEAD to get the current commit hash, appending '-dirty' if there are uncommitted changes."""
    try:
...
def _collect_hidden_imports() -> List[str]:
    ...
    app_plugins_dir = GUI_ROOT.parent / "app_plugins"
    if app_plugins_dir.exists():
        imports.append("app_plugins")
        for path in app_plugins_dir.rglob("*.py"):
            if path.name == "__init__.py":
                continue
            relative = path.relative_to(GUI_ROOT.parent)
